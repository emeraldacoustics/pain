        ...
        if s not in sched:
            ...
            db.update("""
                insert into physician_schedule(
                    user_id,day,time,tstamp,physician_schedule_config_id) 
                values (%s,%s,%s,%s,%s)
...
def cleanUp():
    ...
    vals = []
    o = db.query("""
        select physician_schedule_id from physician_schedule_scheduled
        UNION
        select physician_schedule_id from consultant_schedule_scheduled
        UNION
        select physician_schedule_id from chat_rooms
        """
    )
    for x in o:
        vals.append(str(x['physician_schedule_id']))
    ...
    if len(vals) > 0:
        ...
        for x in vals:
            s.append("%s")
        q = """
            delete from physician_schedule where id not in (%s) and tstamp < now()
            """ % (','.join(vals))
        # print(q)
        db.update(q)
        db.commit()
...
for x in arr:
    ...
    C = 0
    o = db.query("""
            select id,day,time from physician_schedule where user_id=%s 
    """,(x['user_id'],)
    )
    sched = {}
    for g in o:
    ...
    while C < 60:
        ...
        # print("Day: %s / %s" % (C,today))
        C += 1

print("Done. Proceeding to cleanup")
cleanUp()
