...
export const RECEIVED_SUPPS_DATA_SUCCESS = 'RECEIVED_SUPPS_DATA_SUCCESS';
export const RECEIVING_SUPPS_DATA = 'RECEIVING_SUPPS_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_SUPPS_DATA_SUCCESS,
        payload
    }
}

export function supportSave(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({ //eslint-disable-line no-unused-vars
        ...
        }).post('/admin/tickets/update',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_SUPPS_DATA_SUCCESS,
                payload: e.data.data
