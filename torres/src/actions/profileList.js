...
export const RECEIVED_OFLIST_DATA_SUCCESS = 'RECEIVED_OFLIST_DATA_SUCCESS';
export const RECEIVING_OFLIST_DATA = 'RECEIVING_OFLIST_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_OFLIST_DATA_SUCCESS,
        payload
    }
}

export function profileList(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({ //eslint-disable-line no-unused-vars
        ...
        }).post('/office/profile/list',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_OFLIST_DATA_SUCCESS,
                payload: e.data.data
