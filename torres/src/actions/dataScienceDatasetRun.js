...
export const RECEIVED_DSR_DATA_SUCCESS = 'RECEIVED_DSR_DATA_SUCCESS';
export const RECEIVING_DSR_DATA = 'RECEIVING_DSR_DATA';
...
export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_DSR_DATA_SUCCESS,
        payload
    }
}

export function getDataScienceDatasetRun(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({ //eslint-disable-line no-unused-vars
        ...
        }).post('/storage/dataset/run',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_DSR_DATA_SUCCESS,
                payload: e.data.data
