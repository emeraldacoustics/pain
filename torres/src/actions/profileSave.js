...
export const RECEIVED_OFPP_DATA_SUCCESS = 'RECEIVED_OFPP_DATA_SUCCESS';
export const RECEIVING_OFPP_DATA = 'RECEIVING_OFPP_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_OFPP_DATA_SUCCESS,
        payload
    }
}

export function profileSave(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({ //eslint-disable-line no-unused-vars
        ...
        }).post('/office/profile/update',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_OFPP_DATA_SUCCESS,
                payload: e.data.data
