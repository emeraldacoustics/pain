...
export const RECEIVED_PLUPD_DATA_SUCCESS = 'RECEIVED_PLUPD_DATA_SUCCESS';
export const RECEIVING_PLUPD_DATA = 'RECEIVING_PLUPD_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_PLUPD_DATA_SUCCESS,
        payload
    }
}

export function plansListUpdate(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({ //eslint-disable-line no-unused-vars
        ...
        }).post('/admin/plans/update',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_PLUPD_DATA_SUCCESS,
                payload: e.data.data
