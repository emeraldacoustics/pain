...
CREATE PROCEDURE import_qb_user(
  ...
  BEGIN
      ...
      select duration into @dur from pricing_data where id=pricing_data_id;
      ...
      set @insert = concat('update office_plans set  \
                end_date = date_add(start_date,interval ',@dur,' month) where id = ',@op_id);
      select @insert;
      PREPARE insert_q FROM @insert;
      EXECUTE insert_q;
